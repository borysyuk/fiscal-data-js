...
export function getModelFromFDP(fdp) {
  ...
  if (fdp.mapping.measures.length){
    _.map(
      ...
      (measure) => { result.measures[measure.source] = {'currency': measure.currency, 'title': schema[measure.source]} }
    );
  }
  ...
  if (fdp.mapping.dimensions.length){
    _.forEach(
      ...
      (dimension) => {
        _.forEach(
          dimension.fields,
          (field) => { result.dimensions[field.source] = {'type': field.name, 'title': schema[field.source] } }
        )
      }
    );
  }
