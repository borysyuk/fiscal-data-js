...
function makeDefaultUIFromDataState(dataState) {
  ...
  console.log({'selections': {'dimensions': {'filters': filters}, 'measures': measures}});
  return {'selections': {'dimensions': {'filters': filters}, 'measures': measures}};
}
...
export function getCurrentData(state){
  return _.filter(state.data.values, state.ui.selections.dimensions.filters);
}