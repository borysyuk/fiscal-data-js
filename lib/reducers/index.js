...
function stateTree(state = defaultStateTree, action) {
  ...
  switch (action.type) {
    ...
    case SET_DEFAULT_STATE:
      if (action.stateTree) {
        action.stateTree.data.values = simpleDataStorage.set(action.stateTree.data.values);
        action.stateTree.data.flags.isLoaded = true;
      }
      return speedDeepClone(Object.assign({}, state, action.stateTree));
